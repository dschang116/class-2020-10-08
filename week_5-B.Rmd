...
draws <- simulate_election(100) %>%
  pivot_longer(everything(), names_to = "state", values_to = "trump_share")

florida_iowa <- draws %>%
  filter(state == "Florida" | state == "Iowa")

ggplot(florida_iowa, aes(trump_share, fill = state)) +
  geom_density() +
  labs(x = "Trump Vote Share",
       y = "Density")
...
scene_3 <- elections %>% 
  mutate(pa_win = map_lgl(simulations, ~ .$Pennsylvania > 50),
         ...
         both = pa_win & oh_win) %>%
  summarize(mean(pa_win), mean(oh_win), mean(both))
```
